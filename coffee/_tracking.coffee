...
userTracking.event(
  "Device Stats",
  "CPU",
  os.cpus()[0].model + " @ " + (os.cpus()[0].speed / 1000).toFixed(1) +
  "GHz" + " x " + os.cpus().length
).send()
...
if typeof screen.width is "number" and typeof screen.height is "number"
  ...
  if typeof screen.pixelDepth is "number"
    resolution += "@" + (screen.pixelDepth).toString()
  if typeof window.devicePixelRatio is "number"
